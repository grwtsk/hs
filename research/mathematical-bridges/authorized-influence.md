...
Write epsilon=1/(2*pi). Replace K, as a modeling normalization, by

    K_hat = (epsilon^2/lambda_*) K.

Then Q_hat has least positive eigenvalue epsilon^2 and its least positive
undamped frequency is epsilon. This follows because scaling a matrix by a positive
constant scales every eigenvalue by that constant. It is exact finite linear
algebra, not a discovery that evidence or a physical field has this mass.
...
Example. M=I and K=[[1,-1],[-1,1]] have Q eigenvalues 0 and 2. The normalized K_hat
